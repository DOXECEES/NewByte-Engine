#ifndef NBCOMMON_RENDERER_COLOR_HPP
#define NBCOMMON_RENDERER_COLOR_HPP
...
namespace nb
{
        ...
        struct HSV
        {
                  ...
                  value(value)
            {
                ...
                NB_ASSERT(
                    saturation >= 0.0f && saturation <= 1.0f, "SATURATION MUST BE BETWEEN 0 AND 1"
                );
        ...
        struct RGBA
        {
            ...
            NB_NODISCARD constexpr Math::Vector4<float> toVec4() noexcept
            {
                return Math::Vector4<float>(
                    static_cast<float>(r) / 255.0f, static_cast<float>(g) / 255.0f,
                    static_cast<float>(b) / 255.0f, static_cast<float>(alpha) / 255.0f
                );
            }
        ...
        public:
            ...
            NB_NODISCARD constexpr HSV toHsv() const noexcept
            {
                ...
                if (deltaChroma != 0.0f)
                {
                    if (maxChroma == r)
                    {
                        hue *= static_cast<float>(
                            std::fmod(((g - b) / deltaChroma), HUE_SECTORS_COUNT)
                        );
                    }
                    else if (maxChroma == g)
                    {
            ...
            NB_NODISCARD constexpr static Color fromHsv(const HSV& hsv) noexcept
            {
                return fromHsv(hsv.hue, hsv.saturation, hsv.value);
            }


            NB_NODISCARD constexpr static Color fromHsv(
                float h,
                float s,
                float v
            ) noexcept
            {
                ...
                float x =
                    chroma *
                    (1.0f - std::fabs(std::fmod(h / HUE_SECTOR_SIZE_IN_DEGREES, 2.0f) - 1.0f));
            ...
            NB_NODISCARD constexpr static Color fromRgb(
                uint8_t r,
                uint8_t g,
                uint8_t b
            ) noexcept
            ...
            NB_NODISCARD constexpr static Color fromLinearRgb(
                float r,
                float g,
                float b
            ) noexcept
            ...
            NB_NODISCARD constexpr static Color fromRgba(
                uint8_t r,
                uint8_t g,
                uint8_t b,
                uint8_t alpha
            ...
            NB_NODISCARD constexpr static Color fromLinearRgba(
                float r,
                float g,
                float b,
                float alpha
            ...
            constexpr void setAlpha(uint8_t a) noexcept
            {
                rgba.alpha = a;
            }
        ...
        private:
            constexpr explicit Color(
                ...
                uint8_t g,
                uint8_t b,
                uint8_t a = MAX_LINEAR_RGBA_VALUE
            ) noexcept
                : rgba(
                      r,
                      g,
